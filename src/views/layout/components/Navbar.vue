        ...
        <div class="right-menu">
            <div>
                ...
                <el-dialog :modal="false" :title="$t('navbar.changeTitle')" v-model="loginVisible" :width="dialogWidth">
                    <el-input type="text" v-model="title" :placeholder="$t('navbar.inputTitle')"
                        @keyup.enter="handleLoginInfo" />
                    <template #footer>
                ...
                <el-dialog custom-class="ruleDialog" :modal="false" :title="$t('navbar.changeRules')"
                    v-model="rulesVisible" :width="clashDialogWidth" :show-close="false">
                ...
                <el-dialog :modal="false" :title="$t('navbar.versionTitle')" v-model="versionVisible"
                    :width="dialogWidth">
                ...
                <el-dialog :modal="false" :title="$t('navbar.resetTitle')" v-model="resetDayVisible"
                    :width="dialogWidth">
                ...
                <el-dialog :modal="false" :title="$t('navbar.importExport')" v-model="importExportVisible"
                    :width="dialogWidth">
                ...
                <el-dialog :modal="false" :title="$t('navbar.passwordTitle')" v-model="dialogVisible"
                    :width="dialogWidth">
                    <el-form :model="form" :rules="registerRules" ref="form" label-position="left">
                        <el-form-item prop="password1">
                            <el-input name="password1" :type="pwdType" v-model="form.password1"
                                :placeholder="$t('inputPass')" show-password />
                        </el-form-item>
                        <el-form-item prop="password2">
                            <el-input name="password2" :type="pwdType" v-model="form.password2"
...
export default {
    ...
    methods: {
        ...
        async openVersion() {
            console.log('openVersion 被调用')
            try {
                await this.systemVersion()
                console.log('systemVersion 完成，版本数据：', this.versionList)
                this.versionVisible = true
                console.log('versionVisible 设为 true')
            } catch (error) {
                console.error('openVersion 出错：', error)
            }
        },
        async systemVersion() {
