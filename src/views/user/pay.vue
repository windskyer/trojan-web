<template>
    <div class="pay-page">
        <div class="pay-card">
            ...
            <p v-if="isWechat && orderQrImage" class="qrcode-tip">
                {{ $t('user.pay.wechatRecognizeTip') }}
            </p>
            <p v-else-if="orderQrImage" class="qrcode-tip">
                {{ $t('user.pay.wechatOpenTip') }}
            </p>
            ...
            <div class="notice-block">
                ...
                <p class="order-copy" @click="copyOrderName">
                    {{ orderName || '-' }}
                </p>
            ...
            <div v-else>
                <div class="section">
                    ...
                    <p v-if="!orderName" class="empty">
                        {{ $t('user.pay.emptyOrder') }}
                    </p>
                ...
                <div class="section">
                    ...
                    <div class="divider"></div>
                    <p v-if="planRows.length === 0" class="empty">
                        {{ $t('user.pay.emptyPlan') }}
                    </p>
                    <div v-else class="info-list">
                        <p
        ...
        <div
            v-if="previewVisible && orderQrImage"
...
export default {
    ...
    computed: {
        ...
        orderRows() {
            ...
            const rows = [
                {
                    label: this.$t('user.pay.orderName'),
                    value: order.order_name || this.orderName,
                },
                {
                    label: this.$t('user.pay.orderPaidAt'),
                    value: order.paid_at,
                },
                {
                    label: this.$t('user.pay.orderTg'),
                    value: order.tg_username,
                },
        ...
        statusClass() {
            ...
            if (['paid', 'success', 'completed', 'done'].includes(status))
                return 'success'
            if (['pending', 'processing', 'unpaid', 'created'].includes(status))
                return 'pending'
        ...
        planRows() {
            ...
            const rows = [
                ...
                {
                    label: this.$t('user.pay.planPrice'),
                    value: this.formatAmount(plan.price),
                },
                {
                    label: this.$t('user.pay.planPeriod'),
                    value: this.formatDays(plan.duration_days),
                },
                {
                    label: this.$t('user.pay.planTraffic'),
                    value: this.formatTraffic(plan.total_data_gb),
                },
            ]
            return rows.filter(
                (item) =>
        ...
        handleOrderStatus() {
            ...
            if (status === 'success') {
                this.$message.success(
                    `${this.$t('user.pay.orderStatus')}：${this.orderStatusText || '-'}`,
                )
        ...
        async fetchDetail(isPolling = false) {
            ...
            try {
                ...
                if (!this.isSuccess(orderResult)) {
                    this.error =
                        orderResult?.message ||
                        this.$t('user.pay.fetchOrderFail')
                    this.order = null
                    this.plan = null
                    this.$router.replace('/login').catch(() => {})
                    return
                ...
                if (!isPolling) {
                    ...
                    this.plan =
                        planData.plan || planData.info || planData || null
                }
            } catch (error) {
